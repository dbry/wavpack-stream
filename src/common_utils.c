...
// WavpackStreamCloseFile()). Functions here are restricted to those that have few
...
{
    return (uint32_t) WavpackStreamGetNumSamples64 (wpc);
}
...
{
    return (uint32_t) WavpackStreamGetSampleIndex64 (wpc);
}
...
double WavpackStreamGetProgress (WavpackContext *wpc)
{
    if (wpc && wpc->total_samples != -1 && wpc->total_samples != 0)
        return (double) WavpackStreamGetSampleIndex64 (wpc) / wpc->total_samples;
    else
        return -1.0;
}
